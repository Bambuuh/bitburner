...
const float_get_mean = (n) =>
  n.reduce(
    (float_accumulator, float_current) => float_accumulator + float_current
  ) / n.length;
...
const float_get_variance = ({
  ...
  n.reduce(
    (float_accumulator, float_current) =>
      float_accumulator + Math.pow(float_current - m, 2)
  ) / n.length;
...
const float_get_skill_against = ({ object_netscript: n, string_server: s }) => {
  ...
  return (
    (float_skill_hack - n.getServerRequiredHackingLevel(s)) / float_skill_hack
  );
};
 ...
 */
const float_get_server_cash_max = ({ object_netscript: n, string_server: s }) =>
  n.getServerMaxMoney(s);
 ...
 */
const float_get_server_growth = ({ object_netscript: n, string_server: s }) =>
  n.getServerGrowth(s);
...
export const array_get_servers_hackable = (object_netscript) => {
  const servers = object_netscript.scan();

  for (let i = 0; i < servers.length; i++) {
    const nestedServers = object_netscript.scan(servers[i]);
  ...
  return servers.filter(
    (string_server) =>
      ...
      object_netscript.getHackingLevel() >=
        object_netscript.getServerRequiredHackingLevel(string_server) &&
...
export const getServerScore = ({
  ...
  const function_score_correction = b
    ? float_get_mean_normalised_score
    : float_get_standard_score;
